...
Khái niệm:
- *model* (mô hình): là cách gọi khác của hàm số, quá trình train hay huấn luyện model là quá trình tìm các tham số của hàm $f$
...
Phương pháp phi tham số không đặt giả định về dạng của hàm $f$ đây có thể là điểm mạnh so với cách làm bên trên vì hàm $f$ giả định trong phương pháp tham số trên có thể rất khác với hàm $f$ thật. Tuy nhiên, phương pháp này lại yêu cầu lượng qaun sát rất lớn.

Ví dụ: sử dụng thin-plate spline để ước lượng $f$: không đặt ra bất kì một dạng nào trước cho f mà đưa ra một ước lượng gần nhất có thể với dữ liệu. Trong hình dưới đây, sai số ước lượng trên tập train là 0 (*a rough thin-plate spline*). Tuy nhiên, trên thực tế khi fit một thin-plate spline, ta cần đặt ra mức độ mượt (*smoothness*) để tránh bị overfitting như hình
![thin-plate spline](/img/2_6.jpg)
...
- Học không giám sát: $x_i$ không có giá trị $y_i$ tương ứng, ta muốn tìm hiểu về mối quan hệ giữa các giá trị hay các observations với nhau.
    + Tiêu biểu: bài toán phân cụm (*clustering*)
...
**Hồi quy** (*regression*): output $y$ là giá trị định lượng. Ví dụ: dự đoán danh số hay giá nhà, giá đất

**Phân loại** (*classification*): output $y$ là giá trị định tính. Ví dụ: phân loại loài hoa, phân loại xem một bạn học sinh có khả năng trượt hay qua môn, ...
